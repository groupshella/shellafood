...
export const ProductCard = memo(function ProductCard({ item, moduleId }: ProductCardProps) {
    ...
    return (
        <div
            dir="rtl"
            className="flex items-center gap-3 rounded-xl bg-white px-4 py-3 shadow-[0_2px_8px_rgba(0,0,0,0.07)]"
        >
            <Link
                ...
                <div className="mt-1.5 flex items-center gap-2">
                    ...
                    {hasDiscount && (
                        <PriceTag
                            amount={item.price}
                            size="sm"
                            className="text-[11px] leading-none text-gray-400 line-through"
                        />
                    )}
                </div>
            </Link>
