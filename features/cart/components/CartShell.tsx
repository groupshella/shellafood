...
export function CartShell({ title, children, isArabic }: CartShellProps) {
  ...
  const handleClearCart = useCallback(async () => {
    ...
    if (result.success) {
      success(isArabic ? "تم تفريغ السلة" : "Cart cleared successfully");
    } else {
      notifyError(result.message ?? isArabic ? "تعذّر تفريغ السلة" : "Failed to clear cart");
    }
    setIsClearing(false);
    ...
    <div className={SHELL_LAYOUT} dir={isArabic ? "rtl" : "ltr"}>
      <header className={`sticky top-0 z-10 flex items-center justify-between bg-white shadow-[0_1px_0_0_rgba(0,0,0,0.06)] dark:bg-gray-900 dark:shadow-[0_1px_0_0_rgba(255,255,255,0.06)] ${HEADER_PADDING}`}>
        ...
        <div className={`flex items-center justify-center py-3.5 sm:py-4 ${HEADER_PADDING}`}>
          <h1 className="text-base font-bold text-gray-900 dark:text-gray-50 sm:text-lg lg:text-xl">{isArabic ? "سلة التسوق" : "Shopping cart"}</h1>
        </div>
