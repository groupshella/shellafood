...
async function fetchBff<T>(path: string, signal: AbortSignal): Promise<T> {
...
export function useGeneralAnalytics(params: {
    ...
    const { period, initialData = null, enabled = true } = params;
    ...
    useEffect(() => {
        ...
        Promise.all([
            fetchBff<SpendingSummary>(ANALYTICS_BFF.summary, controller.signal),
    ...
    useEffect(() => {
        ...
        fetchBff<SpendingTrend>(path, controller.signal)
            .then((trend) => {
                if (controller.signal.aborted) return;
