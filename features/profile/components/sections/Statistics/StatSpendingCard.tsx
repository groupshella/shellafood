...
export function StatSpendingCard({
	...
	return (
		<div className="flex min-h-[93px] min-w-0 flex-col items-center justify-center gap-[2px] rounded-[8px] bg-card px-3 py-4 shadow-[0px_4px_8.9px_rgba(0,0,0,0.03)] transition-[transform,opacity] duration-150 active:scale-[0.98] active:opacity-95 sm:min-h-24 sm:px-[14px] md:min-h-28">
			{showBadge && (
				<div
					className={[
						"flex h-5 items-center justify-center gap-0.5",
						isNegative ? "text-red-500" : "text-brand",
					].join(" ")}
				>
					<span
			...
			<p
				className="line-clamp-2 text-center text-[12px] font-medium text-foreground sm:text-[14px]"
				style={TAJAWAL}
			>
				{label}
			</p>
