...
export function SpendingChart({
	...
	const chartGeometry = useMemo(() => {
		...
		const points = safeValues.map((value, index) => {
			const x =
				paddingStart +
				(index / Math.max(safeValues.length - 1, 1)) * plotWidth;
			const y =
				paddingTop + plotHeight - (value / CHART_MAX) * plotHeight;
			return { x, y };
		});
			...
			>
				<defs>
					<linearGradient id={gradientId} x1="0" y1="0" x2="0" y2="1">
						<stop
							offset="0%"
							stopColor="rgba(48, 145, 63, 0.132)"
						/>
						<stop
							offset="100%"
							stopColor="rgba(48, 145, 63, 0)"
						/>
					</linearGradient>
				</defs>
				...
				<g
					style={{
						...
						transform:
							showLine && animate ? "scaleX(1)" : "scaleX(0)",
						transition:
							"transform 800ms ease-out, opacity 200ms ease",
