	...
	return [
		{
			id: "bot-1",
			sender: "bot",
			text: isArabic ? "مرحباً بك في شلة" : "Welcome to Shella",
			timestamp: null,
		},
		{
			id: "bot-2",
			sender: "bot",
			text: isArabic ? "كيف أستطيع أن أساعدك؟" : "How can I help you?",
			timestamp: null,
		},
	];
}

export function LiveChatClient({ isArabic }: { isArabic: boolean }) {
	const [messages, setMessages] = useState<ChatMessage[]>(() =>
		createInitialMessages(isArabic),
	);
	...
	return (
		...
		>
			<div className="mx-auto flex min-h-full w-full max-w-lg flex-col justify-end gap-4 px-4 pt-4 sm:max-w-2xl sm:px-5 md:max-w-3xl lg:max-w-4xl lg:px-6">
				<ChatBotAvatar isArabic={isArabic} />
