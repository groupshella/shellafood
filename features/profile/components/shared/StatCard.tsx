...
/** Decorative pastel card fills (illustration accents — not UI chrome tokens). */
...
> = {
	points: {
		...
		wrap: "inset-x-0 bottom-0 h-[58%] sm:h-[60%] md:h-[62%]",
		img: "object-contain object-bottom",
	},
	wallet: {
		src: "/profile/stat-money.png",
		wrap: "inset-x-[6%] bottom-[-2%] h-[52%] sm:h-[54%] md:h-[56%]",
		img: "object-contain object-bottom",
		rotate: "rotate-[4deg]",
	},
	qidha: {
		src: "/profile/stat-coins.png",
		wrap: "inset-x-[12%] bottom-[-4%] h-[52%] sm:h-[55%] md:h-[57%]",
		img: "object-contain object-bottom",
		rotate: "rotate-[18deg]",
	},
};
...
function SarIcon() {
	return (
		<svg
			viewBox="0 0 17 17"
			fill="none"
			className="h-3 w-3 shrink-0 text-foreground xs:h-3.5 xs:w-3.5 sm:h-4 sm:w-4"
			aria-hidden
		>
			<path
		...
		>
			<div className={`relative h-full w-full ${config.rotate ?? ""}`}>
				<Image
					...
					fill
					priority
					sizes="(max-width: 640px) 28vw, (max-width: 768px) 120px, (max-width: 1024px) 140px, 160px"
...
function StatValue({
	...
	return (
		<div className="flex min-h-7 w-full max-w-full items-center justify-center gap-0.5 sm:min-h-8 sm:gap-1">
			...
			<span
				className="truncate text-center text-[clamp(15px,4.2vw,22px)] font-bold leading-none tabular-nums text-foreground sm:text-[clamp(18px,2.4vw,26px)] md:text-[clamp(20px,2vw,28px)]"
				style={AFACAD}
			>
				{formatted}
			</span>
...
export function StatCard({
	...
	const className = [
		...
		"rounded-lg px-1.5 pb-[42%] pt-3 text-center sm:rounded-xl sm:px-2 sm:pb-[40%] sm:pt-3.5 md:rounded-[10px] md:px-3 md:pt-4 lg:pt-5",
		"shadow-[0px_4px_8.9px_rgba(0,0,0,0.03)] transition-[transform,box-shadow] duration-150",
		isInteractive
			? "active:scale-[0.98] md:hover:-translate-y-0.5 md:hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-2 focus-visible:ring-offset-background"
			: "",
	...
	const content = (
		<>
			<span
				className="relative z-10 line-clamp-1 px-0.5 text-[11px] font-bold leading-tight text-foreground xs:text-xs sm:text-[13px] md:text-sm"
				style={TAJAWAL}
			>
				{title}
			</span>
