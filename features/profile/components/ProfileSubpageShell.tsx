...
export function ProfileSubpageShell({
    ...
    return (
        <div className="flex min-h-dvh flex-col overflow-x-hidden bg-white dark:bg-gray-950" dir={isArabic ? "rtl" : "ltr"}>
            <header
                className={`sticky top-0 z-10 grid grid-cols-[auto_1fr_auto] items-center bg-white dark:bg-gray-950 ${elevatedHeader
            ...
            >
                ...
                <h1
                    className={`truncate text-center font-bold leading-[160%] text-[#111B18] dark:text-gray-50 ${elevatedHeader || relaxedHeader ? "text-lg sm:text-[18px]" : "text-base sm:text-[16px]"
                        }`}
                >
                    {title}
                </h1>
            ...
            {subtitle && (
                <p
                    className={`mx-auto w-full max-w-lg px-4 pt-4 text-sm font-bold leading-[160%] text-[#111B18] dark:text-gray-200 sm:max-w-2xl sm:px-5 sm:text-[14px] lg:max-w-4xl lg:px-6 xl:max-w-5xl 2xl:max-w-6xl ${subtitleAlign === "start" ? "text-start" : "text-center"
                        }`}
                >
                    {subtitle}
                </p>
            ...
            <main
                className={`w-full flex-1 overflow-y-auto px-4 sm:px-5 lg:px-6 ${subtitle ? "pb-6 pt-3" : relaxedHeader ? "pb-8 pt-2" : "py-4"
                    } ${mainClassName}`}
            >
                {children}
            </main>
            ...
            {footer && (
                <footer
                    className={`sticky bottom-0 bg-white px-4 pb-2 dark:bg-gray-950 sm:px-5 md:px-6 lg:px-8 ${showFooterBorder ? "border-t border-gray-100 dark:border-gray-800" : ""
                        } ${footerClassName}`}
                >
