...
export function RelatedItemsClient({ items, isArabic }: RelatedItemsClientProps) {
    return (
        <section
            className="bg-background px-3 pt-5 pb-[calc(7rem+env(safe-area-inset-bottom))] sm:px-5 md:pt-6 lg:px-6"
            dir={isArabic ? "rtl" : "ltr"}
            lang={isArabic ? "ar" : "en"}
        >
            <h2 className="mb-3 text-start text-base font-bold text-foreground sm:mb-4 sm:text-lg md:text-xl">
                {isArabic ? "يُباع معها أيضاً" : "Frequently bought together"}
            </h2>
