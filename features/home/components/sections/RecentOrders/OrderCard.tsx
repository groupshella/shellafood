...
export function OrderCard({ order }: { order: RecentOrder }) {
	return (
		<Link
			...
			dir="rtl"
		>
			<div className="relative h-11 w-11 shrink-0 overflow-hidden rounded-lg bg-gray-100 sm:h-[52px] sm:w-[52px] sm:rounded-xl">
				<Image src={order.store_logo} alt={order.store_name} fill className="object-cover" sizes="52px" loading="lazy" />
			</div>
			<div className="min-w-0 flex-1 space-y-1 sm:space-y-1.5">
				<h3 className="line-clamp-1 text-sm font-bold text-gray-900 sm:text-[15px] md:text-base">
					{order.store_name}
				</h3>
				<div className="flex flex-wrap items-center gap-1 sm:gap-1.5">
					{order.module_name && (
						<span className="rounded-full bg-[#E0F7FA] px-2 py-0.5 text-[10px] font-medium text-[#00796B] sm:px-2.5 sm:text-xs">
							{order.module_name}
						</span>
					)}
					{order.module_name && <span className="text-gray-300" aria-hidden>•</span>}
					<span className="text-[10px] text-gray-400 sm:text-xs">{timeAgo(order.order_date)}</span>
				</div>
			</div>
			<ChevronLeft className="h-4 w-4 shrink-0 text-gray-300 sm:h-[18px] sm:w-[18px]" />
		</Link>
	);
}
