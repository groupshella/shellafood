...
const SECTION_HEADING =
	"text-sm font-semibold text-muted sm:text-base lg:text-lg";
...
function ProductCardSkeleton() {
	return (
		<div className="relative flex min-h-[172px] w-full min-w-0 flex-row items-center gap-2 overflow-hidden rounded-lg bg-card sm:min-h-[190px]">
...
function SearchLoadingState({ isArabic }: { isArabic: boolean }) {
	return (
		<div
			className={`space-y-6 sm:space-y-8 ${RESULTS_PADDING}`}
			aria-busy="true"
			aria-label={isArabic ? "جاري البحث" : "Searching"}
		>
			<div className="space-y-3 sm:space-y-4">
				<div className="h-5 w-24 animate-pulse rounded bg-card sm:h-6 sm:w-28" />
			...
			<div className="space-y-3 sm:space-y-4">
				...
				<div className="space-y-2.5 sm:space-y-3">
					{Array.from({ length: 3 }).map((_, index) => (
						<div
							key={index}
							className="h-20 animate-pulse rounded-2xl bg-card sm:h-[5.5rem]"
						/>
					))}
				</div>
			</div>
...
function SearchErrorState({ message }: { message: string }) {
	return (
		<div className="rounded-2xl bg-red-50 px-4 py-5 text-center text-sm font-medium text-red-700 sm:px-6 sm:py-6 sm:text-base">
			{message}
		</div>
	);
}
...
function StoresSection({
	...
	return (
		<section
			aria-label={isArabic ? "نتائج المتاجر" : "Store results"}
...
function ProductsSection({
	...
	return (
		<section
			aria-label={isArabic ? "نتائج المنتجات" : "Product results"}
			className="space-y-3 sm:space-y-4"
		>
			<h2 className={SECTION_HEADING}>
			...
			{hasMore && (
				<button
					...
					className="mt-1 flex w-full items-center justify-center gap-2 rounded-2xl border border-brand py-3 text-sm font-semibold text-brand transition-colors active:bg-brand/5 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-2 focus-visible:ring-offset-background disabled:cursor-not-allowed disabled:opacity-60 sm:mx-auto sm:max-w-md sm:py-3.5 sm:text-[15px] md:max-w-lg lg:max-w-xl"
				>
					{isLoadingMore ? (
						<>
...
export function SearchResultsClient() {
	...
	if (isSearching) {
		return (
			<>
				<SearchLoadingState isArabic={isArabic} />
	...
	if (products.length === 0 && stores.length === 0) {
		return (
			<>
				<SearchEmptyState query={query} isArabic={isArabic} />
				<AddToCart moduleId={moduleId} />
			</>
		);
	}
	...
	return (
		<>
			<div className={`flex flex-col gap-6 sm:gap-8 lg:gap-10 ${RESULTS_PADDING}`}>
				<StoresSection stores={stores} isArabic={isArabic} />
				<ProductsSection
					products={products}
					hasMore={hasMore}
