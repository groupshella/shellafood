    ...
    return (
        <div className="border-b border-gray-100  pb-6 sm:pb-8">
            <div className="flex items-center gap-2 mb-3 sm:mb-4">
                <ImageIcon className="w-5 h-5 sm:w-6 sm:h-6 text-gray-500 flex-shrink-0" />
                <label className="text-sm sm:text-base font-medium text-gray-700 ">
                    {"الصور"} ({images.length}/{MEDIA_LIMITS.MAX_IMAGES})
                </label>
            </div>
            <input
                ref={imageInputRef}
                ...
                <div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 gap-2 sm:gap-3 lg:gap-4">
                    {images.map((img, index) => (
                        <div key={index} className="relative group">
                            <div className="relative aspect-square rounded-lg sm:rounded-xl overflow-hidden border border-gray-200  shadow-sm">
                                <Image
                                    src={img}
                                    alt={`Upload ${index + 1}`}
                                    fill
                    ...
                    {images.length < MEDIA_LIMITS.MAX_IMAGES && (
                        <button
                            type="button"
                            className="aspect-square rounded-lg sm:rounded-xl border-2 border-dashed border-gray-300  bg-gray-50/60  hover:border-green-600  hover:bg-green-50  active:bg-green-50 transition-all flex items-center justify-center touch-manipulation"
                            aria-label={"إضافة صور إضافية"}
                            onClick={() => imageInputRef.current?.click()}
                        >
                            <Upload className="w-4 h-4 sm:w-5 sm:h-5 lg:w-6 lg:h-6 text-gray-500 " />
                        </button>
                    )}
                </div>
            )}
