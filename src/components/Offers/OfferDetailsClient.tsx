...
import { Offer, OfferDriver, OfferBookingData } from "@/types/offer.types";
...
export default function OfferDetailsClient({
	...
	useEffect(() => {
		async function loadOfferData() {
			try {
				...
				// Extract transport type and order type
				let tType = resolvedSearchParams.transport || "motorbike";
				let oType = resolvedSearchParams.type || "one-way";

				if (!resolvedSearchParams.transport && fetchedOffer.link) {
					const match = fetchedOffer.link.match(/\/pickandorder\/(\w+)\//);
					if (match) tType = match[1];
				}

				if (!resolvedSearchParams.type && fetchedOffer.link) {
					const url = new URL(fetchedOffer.link, "http://localhost");
					oType = url.searchParams.get("type") || "one-way";
				}
	...
	const handleBookOffer = useCallback(() => {
		...
		setTimeout(() => {
			router.push(
				`/pickandorder/${transportType}/order/details?type=${orderType}&fromOffer=true`
			);
		}, 300);
