...
import { loadAndConvertOrderData } from "./utils/dataConverter";
...
const AcceptedDriversPage: React.FC<AcceptedDriversPageProps> = ({ transportType }) => {
	...
	const handleViewDetails = (driver: DriverWithPricing, e: React.MouseEvent) => {
		...
		if (typeof window !== "undefined") {
			...
			const returnUrl = encodeURIComponent(`/pickandorder/${transportType}/order/accepted-drivers?type=${orderType}`);
