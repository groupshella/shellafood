...
export default function WaitingDriverPage({ transportType }: WaitingDriverPageProps) {
	...
	const handleTimeout = useCallback(() => {
		setDrivers((prev) => {
			...
			setTimeout(() => {
				...
				router.push(`/pickandorder/${transportType}/order/choose-driver?type=${orderType}&timeout=true&rejectedDriverIds=${rejectedIds}`);
			}, 2000);
	...
	useEffect(() => {
		...
		if (driver.status === "accepted") {
			...
			setTimeout(() => {
				router.push(`/pickandorder/${transportType}/order/accepted-drivers?type=${orderType}&sendToAll=false`);
			}, 1500);
		}
	...
	useEffect(() => {
		...
		if (acceptedDrivers.length > 0) {
			...
			setTimeout(() => {
				router.push(`/pickandorder/${transportType}/order/accepted-drivers?type=${orderType}&sendToAll=${sendToAll}`);
			}, 500);
		} else {
	...
	const handleDriverRejected = useCallback((rejectedDriverId: string) => {
		...
		// Return to choose driver after delay
		setTimeout(() => {
			router.push(`/pickandorder/${transportType}/order/choose-driver?type=${orderType}&rejected=true&rejectedDriverId=${rejectedDriverId}`);
		}, 2000);
