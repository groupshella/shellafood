...
import { canCancelOrder } from "../../lib/utils/orderStatus";
import { getCancelButtonLabel, getReorderButtonLabel, getRatingButtonLabel, hasChatAvailable } from "../../lib/utils/routeHelpers";
...
export default React.memo(function ActionButtons({
	...
	return (
		<>
			...
			<div className={`sm:hidden fixed bottom-0 ${isArabic ? "right-0" : "left-0"} ${isArabic ? "left-0" : "right-0"} bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 shadow-lg z-40 p-4`}>
					...
					{isActive && (
						<>
							{supportPhone && onSupport && (
								<button
									onClick={onSupport}
									className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-green-600 dark:bg-green-500 hover:bg-green-700 dark:hover:bg-green-600 text-white rounded-lg font-semibold text-sm transition-colors shadow-md ${isArabic ? "flex-row-reverse" : ""}`}
								>
							...
							{shouldShowChat && (
								<button
									onClick={onChat}
									className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-white dark:bg-gray-800 border-2 border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-200 rounded-lg font-semibold text-sm hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors ${isArabic ? "flex-row-reverse" : ""}`}
								>
							...
							{onCancel && canCancel && (
								<button
									onClick={onCancel}
									className={`flex items-center justify-center gap-2 px-4 py-3 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 rounded-lg font-semibold text-sm hover:bg-red-200 dark:hover:bg-red-900/50 transition-colors ${isArabic ? "flex-row-reverse" : ""}`}
								>
									<X className="w-4 h-4" />
					...
					{isCompleted && (
						<>
							...
							{onReorder && (
								<button
									onClick={onReorder}
									className={`flex items-center justify-center gap-2 px-4 py-3 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-lg font-semibold text-sm hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors ${isArabic ? "flex-row-reverse" : ""}`}
								>
