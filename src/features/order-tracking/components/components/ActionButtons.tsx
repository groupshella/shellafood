...
export default React.memo(function ActionButtons({
	...
	return (
		<>
			...
			{(
				<div className={`sm:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 shadow-lg z-40 p-4 pb-safe`}>
						...
						{isActive && (
							<>
								...
								{onCancel && canCancel && (
									<button
										onClick={onCancel}
										className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 rounded-lg font-semibold text-sm hover:bg-red-200 dark:hover:bg-red-900/50 transition-colors ${isArabic ? "flex-row-reverse" : ""}`}
									>
										<X className="w-4 h-4" />
										<span>{cancelLabel}</span>
									</button>
