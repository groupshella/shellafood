 ...
 * 
 * @module features/pick-and-order/lib/utils/pricing
 */

import type { PricingBreakdown, OrderPricingData } from '../../types/pick-and-order.types';
 ...
 */
export const calculateTotalDistance = (locationPoints: Array<{ location: { lat: number; lng: number } | null }>): number => {
...
export const calculateOrderPricing = (orderData: OrderPricingData): PricingBreakdown => {
	...
	return calculatePricingBreakdown(basePrice, distance);
};
 ...
 * 
 * @note This is a synchronous utility function. For async API calls, use calculatePricing from the API module.
 */
export const calculatePricingBreakdown = (basePrice: number, distance: number = 0): PricingBreakdown => {
