...
import type { DeliveryAddress } from "@/features/home/types";
...
function useGeolocation() {
					...
					switch (error.code) {
						case error.PERMISSION_DENIED:
							errorMessage = isArabic
								? "تم رفض طلب الوصول إلى موقعك. يرجى السماح بالوصول إلى الموقع في إعدادات المتصفح"
						...
						default:
							errorMessage = isArabic
								? "حدث خطأ أثناء محاولة الحصول على موقعك"
	...
	return { detectLocation, isDetecting };
}

/**
 * Convert Address to DeliveryAddress format
 */
function addressToDeliveryAddress(address: Address): DeliveryAddress {
	return {
		id: address.id,
		label: address.title,
 ...
 * Handles address selection, detection, and management
 */
function AddressSelector({ onAddressChange }: AddressSelectorProps) {
	...
	// Update parent when selected address changes (skip detected addresses to avoid double calls)
	useEffect(() => {
		if (selectedAddress && !selectedAddress.id.startsWith("detected")) {
			onAddressChange?.(selectedAddress);
		}
	}, [selectedAddress, onAddressChange]);

	// Memoized recent addresses
	const recentAddresses = useMemo(() => addresses.slice(0, 3), [addresses]);
	...
	const handleDetectLocation = useCallback(async () => {
		try {
			...
			navigator.geolocation.getCurrentPosition(
				(position) => {
				try {
					// Persist location in a cookie for server-side usage (7 days)
					document.cookie = `location=${encodeURIComponent(JSON.stringify({
						lat: position.coords.latitude,
						lng: position.coords.longitude,
					}))}; path=/; max-age=${7 * 24 * 60 * 60}`;
				} catch (err) {
					console.error('Failed to set location cookie', err);
				}
				
					console.log('lat', position.coords.latitude.toString());
					console.log('lng', position.coords.longitude.toString());
					startTransition(() => {
						router.push(`/home`);
	...
	const handleAddressSave = useCallback(
		...
		[editingAddress, addAddress, updateAddress, onAddressChange]
	);
	...
	return (
		<>
			...
			{/* Map Modal */}
			{viewingAddress && (
				<MapModal isOpen={showMapModal} onClose={handleCloseMapModal} address={viewingAddress} />
			)}
		</>
	);
}
...
const AddressSelectorHeader = memo(({ isArabic, addressesCount, onManageAddresses }: AddressSelectorHeaderProps) => (
...
AddressSelectorHeader.displayName = "AddressSelectorHeader";
...
const SelectedAddressDisplay = memo(({ address, addressesCount, showAddressList, isArabic, onClick }: SelectedAddressDisplayProps) => (
	<motion.div
...
SelectedAddressDisplay.displayName = "SelectedAddressDisplay";
...
const AddressInput = memo(({ value, isArabic, onChange, onFocus, onClear }: AddressInputProps) => (
...
const QuickActions = memo(({ isArabic, isDetecting, recentAddresses, selectedAddress, onDetectLocation, onAddAddress, onSelectAddress }: QuickActionsProps) => (
...
const AddressSuggestions = memo(({ address }: AddressSuggestionsProps) => (
	<motion.div
		initial={{ opacity: 0, y: -10 }}
		animate={{ opacity: 1, y: 0 }}
