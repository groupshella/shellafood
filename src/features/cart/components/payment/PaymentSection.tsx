...
interface PaymentSectionProps {
	...
	onCardDetailsChange?: (details: { number: string; expiry: string; cvv: string; name: string }) => void;
...
export default function PaymentSection({
	...
	const handleExpiryChange = (value: string) => {
		const formatted = formatExpiryDate ? formatExpiryDate(value) : value.replace(/\D/g, '').slice(0, 4);
		onCardDetailsChange?.({
			...cardDetails,
			expiry: formatted,
		});
	...
	const handleCVVChange = (value: string) => {
		const formatted = formatCVV ? formatCVV(value) : value.replace(/\D/g, '').slice(0, 3);
		onCardDetailsChange?.({
			...cardDetails,
			cvv: formatted,
		});
