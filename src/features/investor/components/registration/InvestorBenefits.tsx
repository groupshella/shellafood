...
import { INVESTOR_BENEFITS, BENEFIT_CIRCLE_COLORS } from "../../constants/investor.constants";
...
export default function InvestorBenefits() {
	...
	const currentBenefits = INVESTOR_BENEFITS[language];
					...
					{currentBenefits.map((benefit, index) => (
						<div key={index} className={`relative mb-10 flex w-full items-center justify-between ${index % 2 === 0 ? 'flex-row-reverse' : 'flex-row'}`}>
							...
							<div className="flex items-center justify-center">
								<div className={`w-12 h-12 rounded-full flex items-center justify-center text-white font-bold text-xl bg-gradient-to-r ${BENEFIT_CIRCLE_COLORS[index]} shadow-lg dark:shadow-gray-900/50 transition-transform duration-300 hover:scale-105`}>
									{index + 1}
								</div>
							</div>
