...
import { Star, MapPin, ArrowLeft, CheckCircle, MessageCircle, Package, X, Loader2, Expand, Navigation, Users } from "lucide-react";
import { GoogleMap, Marker, useJsApiLoader } from "@react-google-maps/api";
import { MAP_CONFIG } from "@/lib/maps/utils";
import StepperNavigation from "../Booking/StepperNavigation";
...
// ============================================================================
// WorkerCard Component - Expert UI/UX Implementation
// ============================================================================
 ...
 * 
 * Displays worker information in a card format with:
 * - Avatar with selection indicator
 * - Name and experience
 * - Rating display
 * - Pricing information
 * - Distance and estimated time
 * - Choose button with gradient styling
 * 
 * Features:
 ...
 */
const WorkerCard = memo<WorkerCardProps>(({ 
	worker, 
	isSelected, 
	onSelect, 
	onChoose, 
	...
	const handleChooseClick = useCallback((e: React.MouseEvent<HTMLButtonElement>) => {
		e.stopPropagation();
		e.preventDefault();
		onChoose(worker.id);
	}, [worker.id, onChoose]);
	...
	const hasHalfStar = worker.rating % 1 >= 0.5;

	return (
		<article
			className={`group relative bg-white dark:bg-gray-800 border-2 rounded-xl p-4 sm:p-5 md:p-6 cursor-pointer transition-all duration-300 ease-out ${
				isSelected
					? "border-[#31A342] dark:border-green-500 shadow-lg shadow-[#31A342]/20 dark:shadow-green-500/20 scale-[1.02]"
					: "border-gray-200 dark:border-gray-700 hover:border-[#31A342]/50 dark:hover:border-green-500/50 hover:shadow-md"
			}`}
			onClick={handleCardClick}
			role="button"
			tabIndex={0}
			aria-label={`${worker.name}, ${isArabic ? "خبرة" : "Experience"}: ${worker.experience}, ${isArabic ? "تقييم" : "Rating"}: ${worker.rating}`}
		...
		>
			...
			{isSelected && (
				<div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-[#31A342] to-[#2a8f38] dark:from-green-500 dark:to-green-600 rounded-t-xl" />
			)}
			...
			<div className="flex items-start gap-3 sm:gap-4">
				...
				<div className="relative flex-shrink-0">
					<div className="relative">
						<Image
							src={worker.avatar}
							alt={`${worker.name} ${isArabic ? "صورة" : "avatar"}`}
							width={64}
							height={64}
							className="w-14 h-14 sm:w-16 sm:h-16 rounded-full object-cover ring-2 ring-gray-200 dark:ring-gray-700 group-hover:ring-[#31A342]/50 dark:group-hover:ring-green-500/50 transition-all duration-300"
							loading="lazy"
							priority={false}
						/>
						{/* Selection Badge */}
						{isSelected && (
							<motion.div
								...
								className="absolute -top-1 -right-1 w-6 h-6 sm:w-7 sm:h-7 bg-gradient-to-br from-[#31A342] to-[#2a8f38] dark:from-green-500 dark:to-green-600 rounded-full flex items-center justify-center shadow-lg ring-2 ring-white dark:ring-gray-800"
				...
				<div className="flex-1 min-w-0">
					{/* Header: Name, Experience, and Rating */}
					<div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 sm:gap-3 mb-3">
						{/* Name and Experience */}
						<div className={`flex-1 min-w-0 ${isArabic ? "text-right" : "text-left"}`}>
							<h3 className="font-bold text-gray-900 dark:text-gray-100 text-base sm:text-lg mb-1 truncate">
								{worker.name}
							</h3>
							<div className="flex items-center gap-1.5">
								<span className="text-[#FA9D2B] dark:text-orange-400 text-xs sm:text-sm font-semibold">
						...
						<div className={`flex items-center gap-1.5 ${isArabic ? "flex-row-reverse" : ""}`}>
							<div className="flex items-center gap-0.5" role="img" aria-label={`${isArabic ? "تقييم" : "Rating"}: ${worker.rating} ${isArabic ? "من 5" : "out of 5"}`}>
								{[...Array(5)].map((_, i) => {
										...
										<Star
											...
											className={`w-3.5 h-3.5 sm:w-4 sm:h-4 transition-colors ${
												isFilled
													...
													: isHalf
													? "text-yellow-400 fill-yellow-400/50 dark:text-yellow-500 dark:fill-yellow-500/50"
					...
					{/* Bottom Section: Price, Location Info, and Action */}
					<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pt-3 border-t border-gray-100 dark:border-gray-700">
						...
						<div className={`flex-1 min-w-0 ${isArabic ? "text-right" : "text-left"}`}>
							{/* Price */}
							<div className="flex items-center gap-2 mb-1.5">	<span className="text-xs text-gray-500 dark:text-gray-400">
									{isArabic ? "يبدأ من" : "starts from"}
								</span>
								<span className="text-sm sm:text-base font-bold text-[#31A342] dark:text-green-400">
							...
							{(worker.distance !== undefined || worker.estimatedTime !== undefined) && (
								<div className={`flex items-center gap-2 text-xs text-gray-600 dark:text-gray-400 ${isArabic ? "flex-row-reverse" : ""}`}>
									{worker.distance !== undefined && (
										<div className="flex items-center gap-1">
											<MapPin className="w-3.5 h-3.5" />
											<span>{worker.distance.toFixed(1)} {isArabic ? "كم" : "km"}</span>
										</div>
									)}
									{worker.distance !== undefined && worker.estimatedTime !== undefined && (
									...
									{worker.estimatedTime !== undefined && (
										<div className="flex items-center gap-1">
											...
											<span>{worker.estimatedTime} {isArabic ? "دقيقة" : "min"}</span>
										</div>
									)}
								</div>
							)}
						...
						<button
							...
							className={`relative px-5 sm:px-6 py-2.5 sm:py-3 rounded-lg text-sm font-semibold transition-all duration-300 shadow-sm hover:shadow-md focus:outline-none focus:ring-2 focus:ring-[#31A342] focus:ring-offset-2 dark:focus:ring-green-500 ${
								isSelected
									? "bg-[#31A342] text-white hover:bg-[#2a8f38] dark:bg-green-600 dark:hover:bg-green-700 shadow-[#31A342]/20"
									: "bg-gradient-to-r from-[#31A342] to-[#2a8f38] text-white hover:from-[#2a8f38] hover:to-[#31A342] dark:from-green-600 dark:to-green-700 dark:hover:from-green-700 dark:hover:to-green-600"
							}`}
							aria-label={`${isArabic ? "اختيار" : "Choose"} ${worker.name}`}
							aria-pressed={isSelected}
						>
							<span className="relative z-10">{isArabic ? "اختيار" : "Choose"}</span>
							{/* Button Hover Effect */}
			...
			{/* Hover Glow Effect */}
			<div
				className={`absolute inset-0 rounded-xl bg-[#31A342]/5 dark:bg-green-500/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 -z-10 pointer-events-none ${
...
const ChooseWorker = ({ serviceSlug, serviceTypeSlug }: ChooseWorkerProps) => {
	...
	const handleConfirmWorker = useCallback(() => {
		if (selectedWorker) {
			...
			if (worker) {
				// Store worker data in sessionStorage for waiting-worker page
				if (typeof window !== "undefined") {
					const workerData = {
						id: worker.id,
						name: worker.name,
						nameAr: worker.name, // You can add Arabic name if available
					...
					sessionStorage.setItem(`worker_${worker.id}`, JSON.stringify(workerData));
				}
				...
				// Redirect to waiting-worker page after confirmation
				const waitingWorkerPath = `/serve-me/${serviceSlug}/${serviceTypeSlug}/order/waiting-worker?workerId=${worker.id}`;
				router.prefetch(waitingWorkerPath);
				router.push(waitingWorkerPath);
			}
		}
	...
	const handleSendToAll = useCallback(() => {
		...
		// Store all worker data in sessionStorage
		if (typeof window !== "undefined") {
		...
		// Redirect to waiting-worker page with all worker IDs
		const workerIds = workers.map(w => w.id).join(',');
		const waitingWorkerPath = `/serve-me/${serviceSlug}/${serviceTypeSlug}/order/waiting-worker?workerIds=${workerIds}&sendToAll=true`;
		router.prefetch(waitingWorkerPath);
		router.push(waitingWorkerPath);
	}, [workers, serviceSlug, serviceTypeSlug, router]);
	...
	return (
		<div className={`min-h-screen bg-white dark:bg-gray-900 ${isArabic ? "rtl" : "ltr"}`} dir={isArabic ? "rtl" : "ltr"}>
			...
			<StepperNavigation service={serviceSlug} serviceType={serviceTypeSlug} />
			...
			<div className="flex flex-col lg:flex-row h-[calc(100vh-80px)]">
				...
				<div className="flex-1 overflow-y-auto lg:max-h-full">
					<div className="p-4 lg:p-6">
						...
						{/* Send to All Button */}
						{!isLoadingWorkers && workers.length > 1 && (
							<button
							...
							>
								...
								<span>
									{isArabic 
										? `إرسال الطلب إلى جميع العمال (${workers.length})` 
										: `Send Request to All Workers (${workers.length})`
									}
								</span>
							</button>
						)}
						...
						<div className="space-y-6">
							...
							<div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
								<p className="text-sm sm:text-base text-gray-700 dark:text-gray-300 leading-relaxed">
									{isArabic
										? "هل أنت متأكد من اختيار هذا العامل لإتمام الخدمة؟ سيتم إرسال الطلب وانتظار قبول العامل."
										: "Are you sure you want to select this worker to complete the service? The request will be sent and we'll wait for the worker's acceptance."}
								</p>
							</div>
