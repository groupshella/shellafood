...
import { PAYMENT_METHODS, PaymentMethodId } from "../../../lib/utils/paymentMethods";
import { calculatePricing, formatPrice } from "../../../lib/utils/pricing";

export default function PaymentPage({ service, serviceType }: { service: string; serviceType: string }) {
	...
	// Check for selected worker from accepted workers page and update booking
	useEffect(() => {
		if (typeof window !== "undefined") {
			const selectedWorkerData = sessionStorage.getItem("selectedWorkerForBooking");
			if (selectedWorkerData) {
				try {
					const workerData = JSON.parse(selectedWorkerData);
					updateBooking(workerData);
					sessionStorage.removeItem("selectedWorkerForBooking");
				} catch (error) {
					console.error("Error parsing selected worker data:", error);
				}
			}
		}
	}, [updateBooking]);
