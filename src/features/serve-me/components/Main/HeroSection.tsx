...
export const HeroSection: React.FC<HeroSectionProps> = ({
	...
	return (
		<div className="relative overflow-hidden bg-gradient-to-br from-green-50 via-emerald-50 to-teal-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 py-8 sm:py-12 md:py-16 lg:py-20 xl:py-24">
			...
			<div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16">
				<div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 md:gap-10 lg:gap-12 xl:gap-16 items-center">
					{/* Text Content */}
					<div className={`${isArabic ? "order-2 lg:order-1" : "order-1"} ${isArabic ? "text-right" : "text-left"}`}>
						...
						{/* Enhanced Search Bar */}
						<form onSubmit={handleSearch} className={`relative max-w-2xl mx-auto lg:mx-0 transition-all duration-300 ${isFocused ? "scale-[1.02]" : ""}`}>
							<div className="relative">
								<Search className={`absolute top-1/2 -translate-y-1/2 h-5 w-5 transition-colors duration-200 ${
									isFocused ? "text-green-600 dark:text-green-400" : "text-gray-400 dark:text-gray-500"
								} ${isArabic ? "right-4 md:right-5" : "left-4 md:left-5"}`} strokeWidth={2.5} />
								<input
									type="text"
									value={searchQuery}
					...
					<div className={`${isArabic ? "order-1 lg:order-2" : "order-2"} flex items-center ${isArabic ? "justify-center lg:justify-start" : "justify-center lg:justify-end"}`}>
