...
export default function CarBookingDetailsPage({ serviceId }: { serviceId: string }) {
  ...
  const { models, loading: modelsLoading } = useCarModels(formData.makeId);
  ...
  const handleFormChange = useCallback((e: React.ChangeEvent<any>) => {
    ...
    setFormData((prev: any) => {
      ...
      if (name === 'specailizationId') {
        return { ...prev, [name]: value, ["specailization"]: e.target.options[e.target.options.selectedIndex].label };
      }
      if (name === 'makeId') {
        return { ...prev, [name]: value, modelId: "", ["make"]: e.target.options[e.target.options.selectedIndex].label };
      }
      if (name === 'modelId') {
        return { ...prev, [name]: value, ["model"]: e.target.options[e.target.options.selectedIndex].label };
      }
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-12">
        ...
        <div className="max-w-4xl mx-auto space-y-8 sm:space-y-10 lg:space-y-12">
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
              <div ref={(el) => registerField('make', el)}>
                <FormSelect
                  label="الماركة"
                  name="makeId"
                  options={makes}
              ...
              <div ref={(el) => registerField('model', el)}>
                <FormSelect
                  label="الموديل"
                  name="modelId"
                  options={models}
              ...
              <div ref={(el) => registerField('year', el)}>
                <FormSelect
                  label="السنة"
                  name="year"
              ...
              />
              <div ref={(el) => registerField('transmission', el)}>
                <FormSelect
                  label="نوع ناقل الحركة"
                  name="transmission"
              ...
              <div ref={(el) => registerField('fuelType', el)}>
                <FormSelect
                  label="نوع الوقود"
                  name="fuelType"
