...
function CategoryCard({ module }: CategoryCardProps) {
    ...
    const handleClick = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
        ...
        setTimeout(() => {
            router.push(`/stores?module_id=${module.id}?moduleName=${module.module_name}`, { scroll: true });
        }, 150);
        ...
        >
            ...
            <div className={`relative bg-white dark:bg-gray-800 rounded-xl sm:rounded-2xl p-3 sm:p-4 md:p-6 border-2 transition-all duration-300 h-full ${isNavigating
              ...
              group-hover:scale-110 group-hover:bg-white/30 dark:group-hover:bg-white/15">
                ...
                <div className="text-base sm:text-lg md:text-xl  text-gray-900 dark:text-white mb-1.5 sm:mb-2 group-hover:text-green-600 dark:group-hover:text-green-400 transition-colors line-clamp-2 min-h-[2.5rem] sm:min-h-[3rem]">{module.module_name}</div>
                {/* Arrow */}
                <div className="absolute bottom-0 rtl:left-3 sm:rtl:left-4 md:rtl:left-6 ltr:right-3 sm:ltr:right-4 md:ltr:right-6">
                    <ArrowRight
                        className={`w-6 h-6 md:w-8 md:h-8  transition-all rtl:rotate-180 ${isNavigating
