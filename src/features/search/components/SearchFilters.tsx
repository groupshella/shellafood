					...
					>
						...
						<div className="space-y-6">
							...
							<div>
								...
								<div className="space-y-2">
									{[
										{ label: isArabic ? "أقل من 50" : "Under 50", range: { min: 0, max: 50 } },
										{ label: isArabic ? "50 - 100" : "50 - 100", range: { min: 50, max: 100 } },
										{ label: isArabic ? "100 - 200" : "100 - 200", range: { min: 100, max: 200 } },
										{ label: isArabic ? "أكثر من 200" : "Over 200", range: { min: 200, max: 1000 } },
									].map((option) => {
										const isActive =
											filters.priceRange?.min === option.range.min &&
											filters.priceRange?.max === option.range.max;
										return (
											<button
												key={option.label}
												onClick={() =>
