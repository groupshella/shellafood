/**
 * WorkerAutoSelectConfirmModal Component
 * Full-height sheet modal for worker confirmation with modern design
 * Similar to pick-and-order's AutoSelectConfirmModal
 */
...
interface WorkerAutoSelectConfirmModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: () => void;
  worker: Worker;
...
export default function WorkerAutoSelectConfirmModal({
  isOpen,
  onClose,
  onConfirm,
  worker,
  ...
  service,
  serviceType,
}: WorkerAutoSelectConfirmModalProps) {
  ...
  const handleViewDetails = () => {
    ...
    sessionStorage.setItem("autoSelectModalWorkerId", worker.id);
    router.push(`/worker/${worker.id}?fromModal=true&returnTo=summary&service=${service}&serviceType=${serviceType}`);
  };

  const handleChat = () => {
    sessionStorage.setItem("autoSelectModalOpen", "true");
    sessionStorage.setItem("autoSelectModalWorkerId", worker.id);
    router.push(`/worker/${worker.id}/chat?fromModal=true`);
  };
  ...
  const workerName = isArabic ? worker.nameAr : worker.name;
  const workerExperience = isArabic ? worker.experienceAr : worker.experience;
  const workerSpecialization = isArabic ? worker.specializationAr : worker.specialization;
          ...
          >
            ...
            >
              ...
              <div className="relative p-4 border-b border-gray-200 dark:border-gray-700">
                <button
                  onClick={onClose}
                  className={`absolute top-4 ${isArabic ? "left-4" : "right-4"} p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full transition-colors`}
                  aria-label={isArabic ? "إغلاق" : "Close"}
                >
                  <X className="w-5 h-5 text-gray-600 dark:text-gray-400" />
                </button>

                <div className={`text-center ${isArabic ? "pl-10" : "pr-10"}`}>
                  <div className="inline-flex items-center gap-2 px-3 py-1 bg-green-600 text-white rounded-full mb-2">
                    <span className="text-xs font-semibold">
                      {isArabic ? "أفضل فني" : "Best Match"}
                    </span>
                  </div>
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                    {isArabic
                      ? "راجع تفاصيل الفني والسعر"
                      : "Review worker details and pricing"}
                  </p>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto overscroll-contain">
                <div className="p-5 space-y-5">
                        ...
                        <div className={`flex items-center gap-3 mb-3 flex-wrap `}>
                          <div className="flex items-center gap-1.5 text-sm">
                            ...
                            <span className="font-bold text-gray-900 dark:text-white">
                              {worker.reviewsCount.toLocaleString()}
                            </span>
                        ...
                        {worker.estimatedTime && (
                          <div className={`mt-2 flex items-center gap-2 text-sm `}>
                            ...
                            <span className="font-semibold text-gray-900 dark:text-white">
                              {worker.estimatedTime} {isArabic ? "دقيقة" : "mins"}
                            </span>
                          </div>
                        )}
                      </div>
                  ...
                  <div className="space-y-3">
                    ...
                    {/* Info Note */}
                    <div className={`flex items-start gap-2 p-2 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded text-xs text-gray-600 dark:text-gray-400 ${isArabic ? "flex-row-reverse text-right" : "text-left"}`}>
                      <Info className="w-3.5 h-3.5 flex-shrink-0 mt-0.5" />
                      <p>
                        {isArabic
                          ? "السعر النهائي قد يختلف بناءً على تفاصيل الخدمة الفعلية"
                          : "Final price may vary based on actual service details"}
                      </p>
                    </div>
                  </div>
                </div>
