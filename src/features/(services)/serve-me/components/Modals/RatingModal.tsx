...
import type { RatingModalProps } from "../../types/serve-me.types";
import { RATING_MODAL_TRANSLATIONS, RATING_MODAL_CONFIG } from "../../constants/serve-me.constants";
...
export default function RatingModal({
	...
	const t = RATING_MODAL_TRANSLATIONS[language];

	// Event handlers
	const handleSubmit = useCallback(async () => {
		if (rating < RATING_MODAL_CONFIG.MIN_RATING) return;
		...
		try {
			...
			setTimeout(() => {
				...
				setFeedback("");
				setHoveredRating(0);
			}, RATING_MODAL_CONFIG.SUCCESS_DISPLAY_DURATION);
		} catch (error) {
	...
	const handleClose = useCallback(() => {
		...
		setTimeout(() => {
			...
			setFeedback("");
			setHoveredRating(0);
		}, RATING_MODAL_CONFIG.CLOSE_DELAY);
	}, [isSubmitting, isSuccess, onClose]);

	// Auto-focus textarea after rating is selected
	useEffect(() => {
		if (rating > 0 && rating < RATING_MODAL_CONFIG.MAX_RATING && textareaRef.current && !isSubmitting) {
			const timer = setTimeout(() => {
				textareaRef.current?.focus();
			}, RATING_MODAL_CONFIG.FOCUS_DELAY);
	...
	useEffect(() => {
		...
		const handleKeyDown = (e: KeyboardEvent) => {
			...
			if (e.key === "Enter" && (e.ctrlKey || e.metaKey) && rating >= RATING_MODAL_CONFIG.MIN_RATING && !isSubmitting && !isSuccess) {
				handleSubmit();
			}
		};
	...
	}, [isOpen, rating, isSubmitting, isSuccess, onClose, handleSubmit]);
	...
	const showLowRatingMessage = useMemo(() => 
		rating >= RATING_MODAL_CONFIG.MIN_RATING && rating < RATING_MODAL_CONFIG.LOW_RATING_THRESHOLD, 
		[rating]
	);
	const stars = useMemo(() => 
		Array.from({ length: RATING_MODAL_CONFIG.STAR_COUNT }, (_, i) => i + 1), 
		[]
	);
			...
			{isOpen && (
				<>
					...
					<motion.div
						...
						className="fixed inset-0 z-1 bg-black/60 dark:bg-black/70 backdrop-blur-sm"
						onClick={handleClose}
					/>
							...
							{!isSuccess && (
								<div className="p-6 sm:p-8">
										...
										>
											{driverPhoto ? (
												...
												<div className="w-20 h-20 sm:w-24 sm:h-24 rounded-full bg-gradient-to-br from-blue-100 dark:from-blue-900/30 to-blue-200 dark:to-blue-800/30 flex items-center justify-center shadow-lg">
													{rating >= RATING_MODAL_CONFIG.HIGH_RATING_THRESHOLD ? (
									...
									<motion.button
										...
										disabled={rating < RATING_MODAL_CONFIG.MIN_RATING || isSubmitting}
