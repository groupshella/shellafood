...
import { calculateOrderPricing, loadAndConvertOrderData } from "@/features/(services)/pick-and-order/lib/utils";
								...
								<div className="p-6">
									<div className="flex flex-col sm:flex-row items-start gap-4">
										...
										<div className="relative flex-shrink-0">
											<div className={`w-20 h-20 sm:w-24 sm:h-24 rounded-full overflow-hidden border-4 transition-all ${selectedDriverId === driver.id
											...
											{/* Vehicle Type Badge */}
											<div className={`absolute -bottom-1 left-1/2 -translate-x-1/2 bg-white dark:bg-gray-800 rounded-full p-1.5 shadow-md border-2 ${isMotorbike
												? "border-yellow-500"
												: "border-green-500"
												}`}>
												<VehicleIcon className={`w-4 h-4 ${isMotorbike ? "text-yellow-500" : "text-green-500"
													}`} />
											</div>
										</div>
										...
										<div className="flex-1 min-w-0 w-full">
											...
											<button
												...
												className={`w-full px-4 py-3 text-white rounded-xl font-bold shadow-lg hover:shadow-xl transition-all flex items-center justify-center gap-2 ${isMotorbike
