...
import AutoSelectConfirmModal from "../../modals/AutoSelectConfirmModal";
import { calculateOrderPricing, formatPrice as formatPriceUtil, formatDistance, calculateTotalDistance, calculateDistance } from "@/features/(services)/pick-and-order/lib/utils";
...
export default function OrderSummaryPage({ transportType, orderType }: OrderSummaryPageProps) {
	...
	useEffect(() => {
		...
		import("../../../../lib/utils/dataConverter").then(({ loadAndConvertOrderData, getRouteSegments, isNewFormat }) => {
	...
	const handleEdit = useCallback(() => {
		router.push(`/pickandorder/${transportType}/${orderType}/order/details`);
	...
	const handleChooseDriver = useCallback(() => {
		...
		if (hasPreSelectedDriver && offerBooking?.preSelectedDriver) {
			...
			router.push(`/pickandorder/${transportType}/${orderType}/order/payment?driverId=${preSelectedDriver.id}&fromOffer=true`);
			return;
		}
	...
	const handleConfirmAutoSelect = useCallback(() => {
		...
		setTimeout(() => {
			router.push(`/pickandorder/${transportType}/${orderType}/order/waiting-driver?autoSelect=true&driverId=${selectedDriver.id}`);
		}, 300);
							...
							>
											...
											<div className="flex items-center gap-2 mt-1">
												<div className="flex items-center gap-0.5">
													{[...Array(5)].map((_, i) => (
														<svg
															key={i}
															className={`w-3.5 h-3.5 ${i < Math.floor(selectedDriver.rating)
								...
								<button
									onClick={() => {
										...
										router.push(
											`/pickandorder/${transportType}/${orderType}/order/payment?driverId=${selectedDriver.id}&fromOffer=true`
										);
									}}
