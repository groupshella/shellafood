...
const DriverCard = memo<{
	...
	return (
		<div
			className={`bg-gradient-to-br ${bgGradient} border-2 rounded-2xl p-4 sm:p-5 cursor-pointer transition-all duration-300 ${borderColor} ${isSelected ? "shadow-2xl scale-[1.02] ring-2 ring-offset-2 ring-offset-white dark:ring-offset-gray-900" + (isTruck ? " ring-green-500/50" : " ring-yellow-500/50") : "hover:shadow-xl hover:scale-[1.01]"
				}`}
			...
			<div className="flex flex-col sm:hidden gap-4">
				...
				<div className="flex items-start gap-3">
					...
					<div className="flex-1 min-w-0">
						...
						<div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-semibold ${isTruck
				...
				<div className="flex-1 min-w-0">
					<div className={`flex items-start justify-between mb-3 `}>
						<div className={`${isArabic ? "text-right" : "text-left"}`}>
							...
							<div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-semibold ${isTruck
								? "bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400"
								: "bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400"
								}`}>
...
export default function ChooseDriverPage({ transportType, orderType }: ChooseDriverPageProps) {
	...
	const handleConfirmDriver = useCallback(async () => {
		if (selectedDriver) {
			...
			try {
				// Import utilities
				const { loadAndConvertOrderData, calculateOrderPricing } = await import("@/features/(services)/pick-and-order/lib/utils");
	...
	return (
		<div className={`min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 ${isArabic ? "rtl" : "ltr"}`} dir={isArabic ? "rtl" : "ltr"}>
			<div className="p-4 lg:p-6 xl:p-8">
				...
				<div className="mb-4 sm:mb-6">
					<div className="flex items-center gap-3 sm:gap-4 mb-3">
						<div className={`p-3 sm:p-4 rounded-2xl shadow-md ${isMotorbike
					...
					<div className="px-4 pb-4 lg:px-8 lg:pb-8">
						...
						<div className="flex flex-wrap gap-2 sm:gap-3 mb-4 sm:mb-6">
							{filterButtons.map((filter) => (
								<button
									...
									className={`px-4 sm:px-5 py-2.5 sm:py-3 rounded-xl text-xs sm:text-sm font-semibold transition-all shadow-sm hover:shadow-md ${activeFilter === filter.key
						...
						{!isLoadingDrivers && drivers.length == 0 && (
							<button
								...
								className={`w-full flex items-center justify-center gap-2 px-4 py-3.5 mb-4 sm:mb-6 text-white rounded-xl font-semibold transition-all shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed text-sm sm:text-base ${isArabic ? "flex-row-reverse" : ""
			...
			{showConfirmModal && selectedDriverData && (
				...
				>
					<div
						className={`bg-white dark:bg-gray-800 rounded-t-3xl sm:rounded-2xl shadow-2xl max-w-lg w-full p-5 sm:p-8 max-h-[90vh] overflow-y-auto ${isArabic ? "text-right" : "text-left"
							}`}
					...
					>
						...
						<div className="flex items-center justify-between mb-5 sm:mb-6">
							...
							<div className={`p-2 rounded-lg ${isMotorbike
						...
						{/* Driver Info Card */}
						<div className={`p-4 sm:p-5 rounded-2xl mb-5 sm:mb-6 bg-gradient-to-br ${isMotorbike
								...
								<div className="flex-1 min-w-0">
									...
									<div className="flex items-center gap-1 mb-2">
										{[...Array(5)].map((_, i) => (
											<Star
												key={i}
												className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${i < Math.floor(selectedDriverData.rating)
													? "text-yellow-400 fill-yellow-400"
													: "text-gray-300 dark:text-gray-600"
													}`}
											/>
									...
									<div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-semibold ${isMotorbike
										? "bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400"
										: "bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400"
										}`}>
						...
						<div className={`flex flex-col sm:flex-row gap-2 sm:gap-3 ${isArabic ? "sm:flex-row-reverse" : ""}`}>
							...
							<button
								...
								className={`w-full sm:flex-1 px-5 sm:px-6 py-3 sm:py-3.5 text-white rounded-xl font-bold shadow-lg hover:shadow-xl transition-all flex items-center justify-center gap-2 text-sm sm:text-base ${isMotorbike ? "bg-yellow-500 hover:bg-yellow-600" : "bg-[#31A342] hover:bg-[#2a8f3a]"
									}`}
							>
