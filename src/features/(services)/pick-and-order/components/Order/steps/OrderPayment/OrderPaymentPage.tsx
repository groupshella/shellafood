...
import { calculateOrderPricing, formatPrice, formatDistance, PAYMENT_METHODS, type PaymentMethodId } from "@/features/(services)/pick-and-order/lib/utils";
...
export default function OrderPaymentPage({ transportType, orderType }: OrderPaymentPageProps) {
	...
	useEffect(() => {
		if (typeof window !== "undefined") {
			...
			import("../../../../lib/utils/dataConverter").then(({ loadAndConvertOrderData }) => {
	...
	const handleConfirmAndPay = async () => {
		...
		router.push(`/pickandorder/${transportType}/${orderType}/order/confirm`);
	};
		...
		<div dir={isArabic ? "rtl" : "ltr"} className="min-h-screen bg-gradient-to-br from-gray-50 dark:from-gray-900 via-green-50/20 dark:via-gray-800/50 to-white dark:to-gray-900 py-8 md:py-12 lg:py-16">
			...
			>
				<div className="flex flex-col justify-center min-h-[80vh]">
						...
						{PAYMENT_METHODS.map((method) => {
							...
							return (
								<button
									...
									className={`relative bg-gradient-to-br from-green-50 dark:from-gray-800 to-white dark:to-gray-800 rounded-xl shadow-md border-2 transition-all duration-200 p-6 hover:shadow-lg text-left ${isArabic ? "text-right" : "text-left"
										} ${isSelected
								...
								>
									<div className={`flex items-start gap-4 sm:gap-6 `}>
										{/* Icon */}
										<div className={`flex-shrink-0 w-14 h-14 sm:w-16 sm:h-16 rounded-xl bg-green-100 dark:bg-green-900/30 flex items-center justify-center ${isSelected ? "bg-green-200 dark:bg-green-800/50" : ""
											}`}>
										...
										<div className="flex-1 min-w-0">
											<h3 className={`text-lg sm:text-xl font-bold text-gray-900 dark:text-gray-100 mb-2 ${isSelected ? "text-green-700 dark:text-green-400" : ""
												}`}>
										...
										<div className="flex-shrink-0">
											<div className={`w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center shadow-md transition-all ${isSelected
					...
					{/* Security Info Box */}
					<motion.div variants={itemVariants} className={`bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl p-4 mb-8 flex items-start gap-3 ${isArabic ? "text-right flex-row-reverse" : "text-left"
						}`}>
