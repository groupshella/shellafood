...
import { INFO_SECTION_ITEMS_DATA, TESTIMONIALS_DATA } from "@/features/(services)/pick-and-order/constants/pick-and-order.constants";
...
export default React.memo(function InfoSection({ transportType }: InfoSectionProps) {
	...
	// Transform info items data with localized text
	const infoItems = useMemo(
		() =>
			INFO_SECTION_ITEMS_DATA.map((item) => ({
	...
	// Transform testimonials data with localized text
	const testimonials = useMemo(
		() =>
			TESTIMONIALS_DATA.map((testimonial) => ({
				name: isArabic ? testimonial.name.ar : testimonial.name.en,
				role: isArabic ? testimonial.role.ar : testimonial.role.en,
				rating: testimonial.rating,
				comment: isArabic ? testimonial.comment.ar : testimonial.comment.en,
	...
	return (
		<section className="relative bg-white dark:bg-gray-900 py-20 md:py-32 lg:py-40 xl:py-48 overflow-hidden">
			...
			<div className="w-full max-w-7xl xl:max-w-[1400px] 2xl:max-w-[1600px] mx-auto px-6 md:px-12 lg:px-16 xl:px-24 2xl:px-32 relative z-10">
				...
				<div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 xl:gap-10 2xl:gap-12">
					...
					<div className={`lg:col-span-5 space-y-6 xl:space-y-8 ${isArabic ? "lg:order-1" : "lg:order-2"}`}>
						...
						<div className="grid grid-cols-2 gap-4">
							{infoItems.map((item: { icon: React.ComponentType<{ className?: string }>; title: string; description: string }, index: number) => {
				...
				>
					{testimonials.map((testimonial: { name: string; role: string; rating: number; comment: string; avatar: string }, index: number) => (
						<motion.div
							key={index}
