...
import { TRANSPORT_TYPE_PAGE_FEATURES_DATA } from "@/features/(services)/pick-and-order/constants/pick-and-order.constants";
...
export default React.memo(function FeaturesSection({ transportType }: FeaturesSectionProps) {
	...
	// Transform features data with localized text
	const features = useMemo(
		() =>
			TRANSPORT_TYPE_PAGE_FEATURES_DATA.map((feature: { iconName: string; title: { ar: string; en: string }; description: { ar: string; en: string }; badge: { ar: string | null; en: string | null } }) => ({
				...
				badge: isArabic ? feature.badge.ar : feature.badge.en,
			})),
		[isArabic]
	);
	...
	return (
		<section className="relative bg-gray-50 dark:bg-gray-900 py-20 md:py-32 lg:py-40 xl:py-48 overflow-hidden">
			...
			<div className="w-full max-w-7xl xl:max-w-[1400px] 2xl:max-w-[1600px] mx-auto px-6 md:px-12 lg:px-16 xl:px-24 2xl:px-32 relative z-10">
				...
				>
					{features.map((feature: { icon: React.ComponentType<{ className?: string }>; title: string; description: string; badge: string | null }, index: number) => {
