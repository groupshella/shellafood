...
/** Proxies Laravel `GET .../stores/popular` — Laravel requires `moduleId`. */
export async function GET(req: NextRequest) {
  ...
  const targetUrl = `${API_ROOT}/stores/popular?${forward.toString()}`;
  ...
  try {
    ...
    console.error("[api/stores/popular]", err);
