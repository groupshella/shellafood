...
/**
 * Error Component for Transport Type Page
 * Professional error state with green accent color and retry functionality
 */
export default function TransportTypeError({ error, reset }: ErrorProps) {
	const { language } = useLanguage();
	const isArabic = language === "ar";

	useEffect(() => {
		console.error("Transport type page error:", error);
	}, [error]);

	return (
		<div
			dir={isArabic ? "rtl" : "ltr"}
			className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 via-white to-gray-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 px-4"
		>
			<div className={`text-center max-w-md ${isArabic ? "text-right" : "text-left"}`}>
				...
				<p className="text-lg text-gray-600 dark:text-gray-400 mb-6">
					{isArabic
						? "عذراً، حدث خطأ أثناء تحميل صفحة نوع النقل. يرجى المحاولة مرة أخرى."
						: "Sorry, an error occurred while loading the transport type page. Please try again."}
				</p>
				...
				{/* Action Buttons */}
				<div className={`flex flex-col gap-3 sm:flex-row sm:justify-center ${isArabic ? "sm:flex-row-reverse" : ""}`}>
					<button
						onClick={reset}
					...
					>
						...
						<span>
							{isArabic ? "العودة إلى Pick & Order" : "Back to Pick & Order"}
						</span>
					</Link>
				</div>
			</div>
