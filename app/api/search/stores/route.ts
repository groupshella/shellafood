import { SearchStoresResponse } from "@/features/search/types/search.types";
...
export async function GET(request: Request) {
    ...
    try {
        const backendRes = await fetch(
            `${BACKEND_URL}/api/v1/stores/search?name=${encodeURIComponent(name)}`,
            {
                method: "GET",
                headers: {
        ...
        if (!backendRes.ok) {
            return apiError("Failed to search stores", backendRes.status);
        }

        const data: SearchStoresResponse = await backendRes.json();
        return apiSuccess<SearchStoresResponse>(data, backendRes.status);
    } catch {
        return apiError("Failed to search stores", 500);
    }
}
