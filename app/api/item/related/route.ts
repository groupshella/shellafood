...
export async function GET(request: Request) {
    ...
    try {
        ...
        if (!backendRes.ok) {
            return apiError("Failed to get related items", backendRes.status);
        }

        const data = (await backendRes.json()) as RelatedItem[];
        return apiSuccess<RelatedItem[]>(data, backendRes.status);
    } catch {
        return apiError("Failed to get related items", 500);
    }
}
